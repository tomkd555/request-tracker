...
function IssueForm({ parentKey, copyFrom }: Props): React.JSX.Element {
  ...
  const [summary, setSummary] = useState(source?.summary ?? templates[initialCategory]?.summary ?? "");
  const [description, setDescription] = useState(source?.description ?? templates[initialCategory]?.body ?? "");
