...
import type { Issue, IssueStatus } from "../../../src/shared/types";
...
test("buckets issues into the column matching their status", () => {
  ...
  const cols = columnsOf(issues, ["open", "in_progress"], byKeyDesc);
...
test("orders columns by ISSUE_STATUSES regardless of the statuses argument's order", () => {
  const cols = columnsOf([], ["closed", "open", "in_progress"], byKeyDesc);
...
test("omits a status absent from the filter", () => {
  ...
  const cols = columnsOf(issues, ["open", "in_progress", "resolved"], byKeyDesc);
...
test("caps a column at the limit and reports the rest as overflow", () => {
  ...
  const cols = columnsOf(issues, ["open"], byKeyDesc, 2);
  const col = cols[0];
...
test("sorts by the comparator argument", () => {
  ...
  const cols = columnsOf(issues, ["open"], byKeyAsc);
