...
- Parent issues with child issues, each with status, priority, category, assignee, start and due dates, a Markdown description and the project's custom columns (汎用列: free text or a choice list, never required)
...
- Issue list with filters (status, assignee, reporter, category, due date, mine, awaiting confirmation, keyword, each choice-list custom column), column sort including the custom columns, overdue and due-soon tones, an unseen-update marker and CSV export
...
- Two settings screens: 設定 (name, appearance, due-soon window, poll interval, shared folder; all per machine) and プロジェクト設定 (categories with colours and templates, members, custom columns; shared by the team)
...
```
<root>/
  project.json                         fiscal year start month, categories with their colours and templates, custom column definitions
